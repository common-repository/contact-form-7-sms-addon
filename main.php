...
public function display_panel($form) {
   ...
   */
   public function save_form( $form ) {
       $id = method_exists($form, 'id') ? $form->id() : $form->id;
       $form = $_POST['wpcf7-sms'];

       $form["phone"] = sanitize_text_field(stripslashes($form["phone"]));
       $form["message"] = sanitize_textarea_field(stripslashes($form["message"]));

       update_option('wpcf7_sms_' . $id, $form);
   }
